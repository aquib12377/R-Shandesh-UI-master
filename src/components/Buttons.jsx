...
export default function ButtonTray({ onSelect }) {
  ...
  const handleButtonClick = (buttonId, e) => {
    ...
    // Handle touch devices: toggle submenu on click
    if (isTouchDevice && button?.hasSubmenu) {
    ...
    // For non-touch or non-submenu buttons
    setShowSubmenu(null);
    if (hoverTimeoutRef.current) {
      clearTimeout(hoverTimeoutRef.current);
      hoverTimeoutRef.current = null;
  ...
  // Touch handlers for ripple effect
  ...
  const handleTouchEnd = (e, btn) => {
    ...
    rippleMouseLeave(fakeEvent, btn);
  };
  ...
  return (
    <div className="fixed right-4 top-1/2 transform -translate-y-1/2 z-[99998]" style={{ fontFamily: 'Gotham-Office, sans-serif' }}>
      <div
        ref={containerRef}
        ...
        {buttons.map((button) => (
          <div className="relative" key={button.id}>
            <button
              ...
              className="dropbox-btn font-zap uppercase overflow-hidden text-shadow-lg p-2 px-4 rounded-2xl transition-all duration-300 cursor-pointer relative whitespace-nowrap
      ...
      {(showSubmenu || isAnimating) && createPortal(
        <div
          ref={submenuRef}
        ...
        >
          {showSubmenu && buttons.find(b => b.id === showSubmenu)?.submenuItems?.map((item) => (
            <button
              key={item.id}
              ref={(el) => {
                if (el) buttonRefs.current[`submenu-${item.id}`] = el;
              }}
              onMouseEnter={(e) => !isTouchDevice && rippleMouseMove(e, e.currentTarget)}
              onMouseLeave={(e) => !isTouchDevice && rippleMouseLeave(e, e.currentTarget)}
              onTouchStart={(e) => handleTouchStart(e, e.currentTarget)}
              onTouchEnd={(e) => handleTouchEnd(e, e.currentTarget)}
              onClick={(e) => handleSubmenuItemClick(showSubmenu, item.id, e)}
              className="overflow-hidden uppercase text-shadow-lg text-white p-2 px-3 rounded-md transition-all duration-300 cursor-pointer relative text-left flex-shrink-0 whitespace-nowrap
